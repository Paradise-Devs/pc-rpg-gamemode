...
hook OnPlayerText(playerid, text[])
{
    ...
    else if(GetPlayerRank(playerid) >= PLAYER_RANK_PARADISER && strfind(text, "!", true) == 0 && strlen(text) > 1)
	{
		strdel(text, 0, 1);
		new message[200];
		format(message, 200, "! [{%06x}%s{fe7a39}] %s: {fe7a39}%s", GetPlayerRankColor(playerid) >>> 8, GetPlayerRankName(playerid, true), GetPlayerNamef(playerid), text);
		SendAdminMessage(PLAYER_RANK_PARADISER, 0xfe7a39ff, message);
		return -1;
	}
    else if(GetPlayerRank(playerid) >= PLAYER_RANK_MODERATOR && strfind(text, "@", true) == 0 && strlen(text) > 1)
	{
		strdel(text, 0, 1);
		new message[200];
		...
		SendAdminMessage(PLAYER_RANK_MODERATOR, 0xedededff, message);
		return -1;
	}
