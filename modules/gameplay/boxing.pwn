...
YCMD:desafiar(playerid, params[], help)
{
	if(IsPlayerOfferingChallenge(playerid))
		...
		return SendClientMessage(playerid, COLOR_ERROR, "* Você precisa responder o desafio pendente primeiro.");
	...
	if(IsPlayerInCube(playerid, 755.4205, -5.0985, 1000.0000, 774.6574, 16.0326, 1004.0000) && GetPlayerInterior(playerid) == 5)
	{
		...
		if(sscanf(params, "k<u>", targetid))
			...
			return SendClientMessage(playerid, COLOR_ERROR, "* O jogador está aguardando alguém aceitar seu desafio.");

		else if(GetPlayerChallengeOfferer(targetid) != INVALID_PLAYER_ID)
			return SendClientMessage(playerid, COLOR_ERROR, "* O jogador está decidindo um desafio pendente.");
