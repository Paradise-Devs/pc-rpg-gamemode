			...
			if(GetPlayerVirtualWorld(playerid) == b)
			{
				if(GetBusinessType(b) == 2)
				{
					if(GetBusinessProducts(b) > 0)
					{
						...
						SendClientMessage(playerid, COLOR_SUCCESS, "Você comprou um celular, seu novo número é {0087FA}%i", GetPlayerPhoneNumber(playerid));
		...
		if(IsPlayerInBusiness(playerid, b, 25.0))
		{
			if(GetPlayerVirtualWorld(playerid) == b)
			{
				if(GetBusinessType(b) == 2)
				{
					...
					{
						SendClientMessage(playerid, COLOR_ERROR, "Você não tem dinheiro suficiente. {C8C8C8}[$%i]", credits);
						return 1;
					}

					SendClientMessage(playerid, COLOR_SUCCESS, "Você comprou creditos para seu celular.");
...
YCMD:sms(playerid, params[], help)
{
	...
	{
		SendClientMessage(playerid, COLOR_ERROR, "Você não possui creditos suficientes. {C8C8C8}[$20]");
		return 1;
	}
	...
	foreach(new i: Player)
	{
		if(GetPlayerPhoneNumber(i) == phoneNumber && phoneNumber != 0)
		{
			...
			if(!GetPlayerPhoneState(i))
				return SendClientMessage(playerid, COLOR_ERROR, "O número está fora de área ou fora de serviço.");
	...
	SendClientMessage(playerid, COLOR_ERROR, "não foi possível enviar a mensagem.");
	return 1;
}
...
YCMD:jogarcelular(playerid, params[], help)
{
	...
	SendClientMessage(playerid, COLOR_SUCCESS, "Você jogou seu celular fora.");
...
YCMD:ligar(playerid, params[], help)
{
	...
	if(GetPlayerPhoneCredit(playerid) < 5)
		return SendClientMessage(playerid, COLOR_ERROR, "Você não possui creditos suficientes. {C8C8C8}[$5]");
	...
	foreach(new i: Player)
	{
		if(GetPlayerPhoneNumber(i) == phoneNumber && phoneNumber != 0)
		{
			...
			SendClientMessagef(playerid, 0xFFFF00FF, "*** Chamando... - número: {FFD700}%i{FFFF00}.", phoneNumber);
	...
	SendClientMessage(playerid, COLOR_ERROR, "O número está fora de área ou fora de serviço.");
	return 1;
}
