...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    if(dialogid == DIALOG_ID)
    {
        ...
        if(FloorRequestedBy[listitem] != INVALID_PLAYER_ID || IsFloorInQueue(listitem))
            GameTextForPlayer(playerid, "~r~O elevador ja esta vindo", 3500, 4);
				...
				if (i <= 12)
				{
				    ...
				    if (ElevatorState != ELEVATOR_STATE_MOVING)
				    {
				        ...
				        if (ElevatorFloor == i + 1)
				        {
							...
							GameTextForPlayer(playerid, "~n~~n~~n~~n~~n~~n~~n~~y~~h~O elevador ja esta~n~~y~~h~neste andar", 3500, 3);

							// Display chat text message to the player
	                        SendClientMessage(playerid, COLOR_MESSAGE_YELLOW, "* O elevador ja esta neste andar... entre e pressione '{FFFFFF}~k~~CONVERSATION_YES~{CCCCCC}'");
					...
					if (ElevatorState == ELEVATOR_STATE_MOVING)
					{
						...
						format(strTempString, sizeof(strTempString), "* O elevador foi chamado... atualmente esta indo para o %s.", FloorNames[ElevatorFloor]);
					}
					else
					{
					    ...
					    {
					    	// Format chat text message
							format(strTempString, sizeof(strTempString), "* O elevador foi chamado... atualmente esta no %s.", FloorNames[ElevatorFloor]);
						}
						else
						{
					    	// Format chat text message
